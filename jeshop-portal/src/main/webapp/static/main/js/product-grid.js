//Jquery
// $(function(){})   页面加载触发
//  $(document).ready(function() {} 页面加载完毕触发
$(document).ready(function() {
    ...
    //通过JavaScript的函数栏进行url的选择。
    var url = switchURL(ctx, ptype);
	...
	$.ajax({
		...
		success : function(result) {
			...
			// 第一步：将JSON串转化为JSON对象，但result本身就是一个对象
			// 第二步：遍历LIst中的对象
			// $.each() 第一个参数是循环的对象 ， 第二个参数对对象中的每一个元素 执行 function函数 ，
			// function 的第一个参数 i 是索引，item 是 循环对象中的每一个元素。
...
//在外部创建函数框，等待参数，之后再JQuery中调用函数即可完成。
...
function switchURL(ctx, ptype){
     ...
     *	    4: 显示bilibili 	            ctx + "/product/selProductForType"
	 *      5：显示 模糊查询 商品			ctx + "/product/selProductForType"
	 **/
	if(ptype == "0"){
